...
\begin{itemize}
    ...
    \item \textbf{LiDAR 2D :} Non résolu. Le diagnostic a identifié un port micro USB arraché sur la carte Radar\_Con. Un branchement de secours via USB-C pour assurer le rôle de ce branchement arraché va être tenté.
...
Afin d'exploiter pleinement le GPU de la Jetson Orin Nano pour la détection de fissures, une chaîne d'optimisation complète a été mise en place. Une image Docker spécialisée (\texttt{Dockerfile.jetson}), basée sur \texttt{dustynv/l4t-pytorch:r36.2.0}, a été construite pour isoler l'environnement de détection. Cette image intègre ROS~2 Humble, PyTorch avec CUDA et la bibliothèque Ultralytics, garantissant la compatibilité GPU native sur la plateforme Jetson.
...
\begin{itemize}
    \item \texttt{\(\pi/2\)} pour le montage original (hérité de l'année dernière);
    \item \texttt{0.0} pour la création de carte en SLAM (LiDAR aligné avec l'avant du robot).
\end{itemize}
