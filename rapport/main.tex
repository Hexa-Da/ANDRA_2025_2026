...
\paragraph{LiDAR 2D}
Un LiDAR (Light Detection and Ranging) qui utilise des impulsions laser pour mesurer des distances avec une grande précision. En émettant des faisceaux lumineux et en calculant le temps qu'ils mettent à revenir après avoir réfléchi sur des objets, ce genre de capteur est capable de reconstituer en deux ou trois dimensions l’environnement, même dans des conditions de faible luminosité. 
...
Contrairement à l'année dernière, l'équipe 2025-2026 n'a pas eu à construire la tourelle du robot, mais nous avons dû reconstitution l'architecture logicielle suite à la perte de l'image système du robot. La priorité a donc été de rétablir la communication entre l'unité de calcul (\textbf{Jetson Orin Nano}) et les différents actionneurs et capteurs.
...
\begin{itemize} 
    \item \textbf{Caméra ZED2i :} La caméra est fonctionnelle et publie correctement les données visuelles et inertielles (IMU), qui ont été intégrées à la configuration du filtre de Kalman étendu (EKF). Le package est installé et la caméra est détectée, mais les tests ne sont pour l'instant pas concluant.
...
\begin{itemize}
    ...
    \item \textbf{Hotspot Jetson :} Configuré et validé. Le robot crée automatiquement un réseau Wi-Fi \texttt{JetsonWIFI} lorsqu'il ne capte pas le réseau du TechLab, permettant la connexion SSH en galerie via \texttt{ssh techlab@orin2.local}.
\end{itemize}
...
\paragraph{Contraintes et limitations :} Malgré ces avancées, plusieurs défis subsistent à l'approche de cette première descente :

\begin{itemize}
    \item \textbf{Absence du LiDAR :} Le capteur LiDAR n'étant pas encore fonctionnel, la navigation se fera sans cartographie ni détection d'obstacles laser, limitant les tests à la séquence de prise de photos.
    \item \textbf{Disponibilité du matériel 360° :} L'équipe ne dispose pas de sa propre caméra 360° intégrée au robot. Pour cette session d'acquisition, nous nous appuierons sur un équipement fourni ponctuellement par l'ANDRA, ce qui repousse l'intégration matérielle définitive.
    \item \textbf{Données ZED2i non vérifiées :} La cohérence des données inertielles et visuelles de la caméra ZED2i n'a pas encore été validée, ce qui limite la fiabilité du filtre de Kalman étendu pour le positionnement.
\end{itemize}
...
Le second semestre s'ouvre sur les enseignements tirés de la première descente et vise à faire progresser le robot vers un système pleinement opérationnel. Les travaux se structurent autour de trois axes : la résolution des problèmes matériels restants, l'amélioration du modèle de détection et la compréhension du système de navigation.
...
Le dataset de la caméra 360° capturé lors de la première descente constitue la matière première du travail d'amélioration du modèle de détection mené par Lucas et Adrien. Les étapes en cours sont :
...
\subsection{Amélioration de la séquence de capture}

Les constats de la première descente ont mis en évidence que la séquence de 3 photos PTZ ne couvre pas l'intégralité de l'arche de la galerie. Plusieurs pistes sont explorées pour améliorer la couverture :
\begin{itemize}
    \item Augmenter le nombre de captures par arrêt (passage de 3 à 5 positions PTZ).
    \item Revoir l'enchaînement des positions de la caméra pour maximiser la couverture.
    \item Expérimenter le script vidéo à très basse vitesse.
\end{itemize}

\subsection{Optimisation YOLO avec Docker et TensorRT}
...
Un script de conversion (\texttt{scripts/convert\_to\_tensorrt.sh}) permet de transformer le modèle PyTorch \texttt{best.pt} en un moteur TensorRT \texttt{best.engine}, offrant une accélération de l'inférence de l'ordre de 3 à 5 fois par rapport au modèle PyTorch original grâce à l'optimisation en demi-précision (half precision). Le code du nœud \texttt{image\_subscriber} a été adapté pour détecter et utiliser automatiquement le moteur TensorRT s'il est disponible, avec un repli transparent sur le modèle PyTorch.
...
\subsection{Amélioration du positionnement et de la navigation}

Le second semestre vise également à faire progresser le système de navigation autonome et de positionnement. Les axes de travail sont :

\begin{itemize}
    \item \textbf{Validation des transformations TF :} Corriger le problème de trajectoire non planaire observé dans RViz2, qui traduit une incohérence dans l'arbre des transformations géométriques.
    \item \textbf{Vérification de la ZED2i :} Valider la cohérence des données inertielles et visuelles de la caméra stéréoscopique, condition préalable à un filtre de Kalman étendu fiable.
    \item \textbf{Analyse des données LiDAR :} Une étape clé consiste à approfondir la compréhension des données générées par le LiDAR, afin de garantir la fiabilité des mesures utilisées pour la navigation et l’élaboration des cartes.
    \item \textbf{Cartographie et réglages SLAM :} La génération de cartes par SLAM est fonctionnelle, mais il reste à en maîtriser parfaitement le paramétrage et l’interprétation, afin d’optimiser la qualité des cartes produites et d’assurer leur cohérence avec l’environnement réel.
    \item \textbf{Localisation AMCL sur cartes SLAM :} L’exploitation des cartes obtenues via SLAM pour la localisation avec AMCL constitue la prochaine étape. Maintenant que le LiDAR est pleinement opérationnel, la mise en œuvre et l’ajustement d’AMCL doivent permettre une localisation précise du robot lors de ses déplacements, conformément aux recommandations de l’équipe 2024-2025.
\end{itemize}
