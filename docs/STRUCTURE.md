...
**Ce qui n’est en principe pas dans Git** : le fichier `backup_robot_usine.img.zst` (très volumineux). La copie de référence pour l’équipe est livrée sur une **clé USB transmise avec le robot** ; d’autres copies peuvent exister sur disque ou NAS. Avant restauration, vérifiez l’intégrité avec `sha256sum` comme dans `docs/BACKUP.md`.
